...
## Branch review (/code-review high) — six findings, five fixed, one recorded

**1. `sentences()` moved more than `sentence_length` (medium, fixed).** `paragraph_length` counts sentences per paragraph and `active_voice` iterates sentences, so both re-segment too. Worse, the newline split defeated `paragraph_length`'s own list exemption: it tested only a paragraph's FIRST character, so a lead-in line followed by bullets in one block was graded as prose, and once the bullets became separate sentences a five-item list failed a cap of four (verified: score 1.0 → 0.0 on a lead-in + 5 bullets). Fixed by testing every line of the paragraph. Three exemption variants were measured over the 540 rows the check scores; the chosen one moves the mean 97.21 → 97.44 on 5 rows, against 36 rows for a tighter variant that also changed the first-line rule. Published `paragraph_length` figures are unchanged (100.0/100.0/100.0/95.8 by tier on agentic-read-report; 16.7 on Fable's agentic-fix-verify). The ledger claim 'at most 1.07 points ... all of it in sentence_length' was **wrong on both counts** and is corrected to 1.50 points across three checks.

**2. `code_block_size` and `no_jargon` read the wrong view (medium, fixed).** Both are ban lists, and the stated criterion says bans read `trace` — I wrote the criterion and then did not apply it. Concretely: with `maxCodeLines: 0`, a beginner-style cell that dumps a 30-line diff in a pre-tool block and then reports cleanly scored 1.0 where the glued turn scored 0. These are the two most heavily weighted beginner checks (weight 2 each). `no_filler` moved with them for the same reason. Costs no re-derivation: on a saved row `final === trace`, so no published figure moves — confirmed by re-running the full 61-run comparison. The rationale comment is rewritten as an explicit ban/rate distinction.

**3. An unrecognised `judgeOn` awarded a free 1.0 (medium, fixed).** `views?.[typo] ?? ''` fell through to the empty-text guard and returned `{score: 1}` — the whole judge weight, 0.3-0.5 of the cell. The check path already throws on an unknown `reads`, and the config test only covers agentic cases, so a typo on a conversational case would have shipped green. `judge` now validates against `VIEWS` and throws.

**4. A non-errored cell with no text scores near-perfect (medium, NOT fixed — out of scope).** `evaluate.mjs`'s guard is `run.error && !run.text`, so only an *errored* empty reply is zeroed. `scoreDeterministic` on an empty string returns **0.931** for beginner on `agentic-fix-verify` (verified), and the judge also early-returns 1.0. **Pre-existing and unchanged by this task** — the guard and the empty-text path are byte-identical on `dev`, and `splitTurn` produces `''` in exactly the cases the old code did. COS-11 owns both halves of this bias. What IS fixed here is the false statement it contradicted: `harness/README.md` said 'a cell that returns no text scores 0 on every rule', which is true only because of the error guard; corrected, with the non-errored case named and pointed at COS-11.

**5. `judgeReads` was written even when the judge never ran (low, fixed).** A `--no-judge` or errored row recorded `judgeReads: 'final'` beside a synthetic 1.0, which is exactly the provenance confusion this change exists to remove. Now `null` when the judge was skipped.

**6. `score` crashed on rows whose style has no contract (low, fixed).** Independently found and recorded earlier this session as out of scope; the review noted the block was rewritten here and that the adjacent line already guards with `?.`. Reconsidered and fixed, because AC #5's own claim is that a saved run re-scores offline for free, and the command could not do it on six saved directories including both arms of the sentence-cap decision. Such rows now carry their saved scores and `score` names the skipped style ids and counts. Verified on `22-27-15`: exit 0, '12 of 24 rows name a style with no contract'.

Each of findings 1, 2 and 3 has a test that fails against the pre-fix code — confirmed by reverting `src/checks.mjs` and `src/judge.mjs` and re-running: 3 failures, named. Suite 90 → 94.
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
Split the assistant's text blocks apart and made every scorer name the string it grades. `run.mjs` no longer concatenates blocks with `text += b.text`: it keeps them in order and `splitTurn` derives two views of a turn — `trace` (all text blocks, blank-line joined) and `final` (the blocks after the last tool call). Every check declares `reads: 'final' | 'trace'` and every agentic case declares `judgeOn`, both enforced by tests and both saved on the row, so a figure lifted from `rows.json` carries the string that produced it. The line is ban vs rate: a check measuring how the answer is written reads the final message, a ban list reads the whole turn, because filler, celebration, emoji, narration, jargon and code are violations wherever the reader sees them. `agentic-read-report`'s judge reads the trace, since its rubric grades narration of the search. `sentences()` also splits on a single newline now, so a list header is no longer merged into its first item.

Verified on real cells. Run `06-21-53` (2 cells, $0.38) caught an Opus cell opening "I'll look at the file first." before 8 tool calls: the glued string the old code would have saved reads one 22-word opening sentence, over beginner's 20-word cap; `final` reads it at 17 words, under it; `trace` reads the narration as its own 6-word sentence. Run `06-21-03` (1 cell, $0.03) confirmed the no-narration case returns both views byte-identical. Every new test was checked against the pre-fix source and fails there.

The fix does not reach backwards, and that is stated rather than papered over. Gluing is lossy, so no saved agentic figure can be re-derived onto it; each was re-scored to confirm the applicable half leaves it unchanged and then explicitly labelled as measured on the glued turn in FINDINGS.md, the experiment ledger, harness/README.md, the audience-level spec and three story docs, and `score` now counts the rows it re-grades that predate the fix. The newline half is re-derivable and was re-derived across all 61 saved runs: it moves rule totals by at most 1.50 points anywhere and 0.00-0.34 on every run behind a published four-tier figure, across three checks rather than one. Segmentation figures move more and were corrected in place after each pre-fix figure was reproduced exactly. That reproduction check found one conclusion needing restatement — the ledger's argument that a 12-word cap did nothing rested on the tightened arm sitting inside the untightened range, and re-segmented it sits above it; same conclusion, firmer, rewritten rather than left standing.

`/code-review high` raised six findings, all real. Five fixed on the branch, including two ways a cell could earn free credit that this change created or left open: `paragraph_length`'s list exemption saw only a paragraph's first character, so once bullets became sentences a five-item list failed a cap of four; and `code_block_size`/`no_jargon` were reading the final message despite being ban lists, so a 30-line diff dumped before the last tool call scored clean on the two heaviest beginner checks. The sixth — a non-errored cell with no text scoring 0.931 on rules — is pre-existing, byte-identical on dev, and recorded for COS-11; the README claim it contradicted was corrected here.

Gates: npm --prefix harness test 79 -> 94, audit exit 0, lore check exit 0. Spend $0.4143.
