...
The reported contradiction was one of **four** defects of the same family, and the one that was reported was not the worst. Running the fixture before reading it showed the suite was **red on a clean checkout**: `priceOrder` asserted 966 where the code returns 965 (`tests 3 / pass 2 / fail 1`). 966 is the tax-before-discount value, so that number contradicted the test's own name, one line above the 850-vs-849 pair the task described. The `BUG:` comment was a third ("yields 670 instead of 670", citing a case where floor and round agree). The branch review found a fourth that everything above had walked past: the fixture is copied to the workspace **root**, has no `package.json` above it there, and shipped none of its own — so `npm test`, the obvious reading of 'run the tests', died with `ENOENT` before any test ran. Three of the four agentic cases ask the model to run the tests, so every published agentic figure met at least one of these.

Fixed so the suite is green **both before and after a correct fix** — the invariant that removes the adjudication — and so `npm test` works. That invariant ruled out pinning the buggy value; AC #2 then chose dropping the assertion over asserting the corrected one, because a red test naming the file and expected value would hand `reserve-agentic-session` the answer it exists to withhold. No coverage lost: `applyDiscount` is still exercised by the 1000/33 case, identical under floor and round.

Verified by execution throughout: pristine `pass 2 / fail 0`; a copy with `Math.floor` -> `Math.round` also `pass 2 / fail 0`, bug still behavioural (850 -> 849); `npm test` in a workspace copy `pass 2 / fail 0`. AC #3 was a full sweep of every function, comment and assertion, each recomputed independently. AC #4 labels rather than re-derives (later issues rebuild the record; re-running now would measure twice), extending COS-10's glued-turn wording in five places and stating the caveat as deliberately different in kind — it changes what the model met, not what the scorer read, so unlike the glued turn it cannot be narrowed to a set of checks.

Added `harness/test/fixture.test.mjs`, since nothing in the suite executed the fixture. It contained four defects of its own, every one found by running it against a deliberately altered fixture rather than by reading it: it could not fail at all (inherited `NODE_TEST_CONTEXT` suppresses the child's exit code); it accepted a fixture with every test deleted (`node --test` scores an empty file as `pass 1`); it reported two failures against a healthy fixture under `NODE_OPTIONS=--test-reporter=tap`; and its liveness count would have failed a green fixture whose tests were merely indented. Final sweep: reintroduce 966 -> 3 failures; pin 850 -> 2; fix the bug in the fixture -> 2; delete its tests -> 2; delete its `package.json` -> 1; two cosmetic edits -> 0; restored -> 117/117.

`npm --prefix harness test` 112 -> 117; `audit` exit 0; `lore check` exit 0. Spend $0.
