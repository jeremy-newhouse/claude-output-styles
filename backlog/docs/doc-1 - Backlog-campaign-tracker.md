...
- **COS-4** asks beginner's judge score to exceed 70%. The best figure in the
  ...
  and not a model one.
- **A ruling the user owes both tasks before either starts.** The owning story's
  acceptance criteria say the bars must be met "on both models". They were
  written when two tiers were measured; four are now, so "both models" can mean
  the original Opus and Sonnet or every tier. Beginner has to climb to 70% from
  45.9 under the first reading and from 37.5 under the second. COS-7 recorded the
  ambiguity in the story and deliberately did **not** move the bar.
