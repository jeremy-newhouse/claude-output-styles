...
this class of drift — number agrees, basis doesn't — now fails the audit
automatically. See Campaign 2 Resolved row 14 and the session log below.
...
| 14 | COS-28 | Task Done — 2026-08-31, session 37 | All 3 ACs verified. **AC #1**: argued and confirmed only advanced claims code counts toward `maxUpdateWords` ("headers and code included"); beginner and intermediate state no basis at all, consistent with `total_length`'s real behavior (`words(stripCode(text))` — fenced code dropped, inline code counted as one placeholder word). Fixed the prose, not the checker: changing `total_length` would move every published figure on all three styles. **AC #2**: paired 22-cell confirmatory arm (11 conversational cases x opus/sonnet x 1 repeat), old prose vs new, same session to avoid confounds — `results/2026-08-31T18-56-26-946Z` (before, 87.6%) vs `results/2026-08-31T18-52-49-055Z` (after, 85.4%). `interval`: composite -2.2 [-5.5,+1.1], judge -6.1 [-15.8,+3.5], rules -0.5 [-2.1,+1.0], words +4.3 [-10.3,+19.0] — every interval crosses zero, no regression at this power; not a full 207-cell precision arm, per the task's own note that a wording-only fix needs only a confirmatory one. **AC #3**: extended `contract-audit.mjs` with `statedBasis()`/`BASIS_CLAIMS`, mirroring the existing `CONDITION_FIELDS` mechanism for `maxCodeLines` — a style claiming code counts toward `maxUpdateWords` now fails the audit even when the number agrees with `contracts.json`. `/code-review high` on the branch found the phrasing list too narrow and the condition/basis matching loop duplicated; both fixed and re-verified (broadened to 4 patterns, checked against the shipped sentence as a false-positive guard; loop extracted to `firstSegmentMatch`). 6 new tests total. `FORCE_COLOR=0 npm --prefix harness test` 231/231 (225 baseline + 6 new); `node src/cli.mjs audit` 12/12 checks agree. `docs/specs/style-contracts-by-audience-level.md`'s existing "filed rather than fixed" note updated with the resolution and the arm's numbers; `lore check` exit 0, 24 files. Raised, not fixed: `capSegments` splits on a bare newline, so a hard-wrapped cap sentence would defeat this check and the pre-existing `ON_REQUEST` check — pre-existing, out of scope. |
...
- 2026-08-31 — session 37: restored, verified no drift (`dev`/`main` both at
  `6807428`, pushed, level; no leftover branches or PRs; cursor and task
  status agreed). **Resolved COS-28, item 21.** Argued and confirmed only
  advanced's prose was wrong (it claimed code counted toward the 120-word
  cap; `total_length` excludes code via `stripCode`), fixed the sentence
  rather than the checker, ran a paired 22-cell confirmatory arm (old vs new
  prose, opus+sonnet, 11 conversational cases, same session) that found no
  regression on composite/judge/rules/words, and extended
  `contract-audit.mjs` with a basis check so a style claiming code counts
  toward a word cap now fails the audit even when the number agrees — the
  same silent-agreement failure mode COS-15 closed for `maxCodeLines`.
  `/code-review high` on the branch found two real gaps in the basis check —
  the phrasing list was too narrow (silently passed "code is also included" /
  "code counts toward it" / two more) and `statedCondition`/`statedBasis`
  duplicated their matching loop — both fixed and re-verified before merge
  (broadened to 4 patterns, checked against the shipped sentence as a
  false-positive guard; loop extracted to `firstSegmentMatch`). **Raised, not
  fixed — out of scope for this task:** `capSegments` splits cap sentences on
  a bare newline, so a style file that hard-wraps mid-sentence inside its cap
  statement would silently defeat this basis check and the pre-existing
  `ON_REQUEST` condition check it shares the mechanism with. Pre-existing,
  not introduced by COS-28; no shipped file wraps that way today. Gates:
  `npm --prefix harness test` 231/231 (`FORCE_COLOR=0`, 6 new tests total);
  `audit` exit 0; `lore check` exit 0, 24 files. `git diff dev...feature/COS-28`
  touches `plain-english-advanced.md`, `harness/src/contract-audit.mjs`,
