...
| `19-42-55` | 300 | beginner only, both models, the five shared cases, 30 repeats — COS-16's before arm | The largest arm on COS-4's pass-1 text, which shipped until COS-16 replaced it: rules **98.9 / 97.4**, judge **66.8 / 60.9**, 0 errors. Opus falls 7.1 from COS-4's 35-cell reading of 73.9; Sonnet rises 2.8 from 58.1. Both land inside COS-4's stated interval — the 35-cell estimate was imprecise, not wrong. |
...
| `2026-08-20T03-03-26` | 300 | beginner, both models, the five shared cases, 30 repeats, E1+E2+E3 with E4 dropped — **the arm the shipped text rests on** | The clean re-run `02-43-48` needed: 150 cells a model, **0 errored**. Rules **99.2 / 97.4**, judge **66.1 / 60.9**, mean reply 62.9 / 58.7 words. Paired against `19-42-55`: rules +0.1 [−0.3, +0.5], judge −0.3 [−3.5, +3.0], composite −0.1 [−1.8, +1.6], words −0.6 [−1.8, +0.7]. Per model, Opus words **−0.12 [−2.28, +2.04]** — the four-edit bundle's +1.71 [+0.27, +3.16] (`20-05-05`) is gone, on a full Sonnet arm this time and not Opus alone. Every interval, pooled and per-model, contains zero. |
| `03-13-05` | 300 | beginner, both models, the six reserve cases, 25 repeats, same candidate — COS-16's AC #5 arm | 150 cells a model, **0 errored**; the first reserve reading on any three-edit candidate. Rules 97.0 / 95.2, judge 65.4 / 58.1, mean reply 79.4 / 80.9 words. Paired against `19-53-49`: rules −0.5 [−1.3, +0.4], judge −0.9 [−5.3, +3.6], composite −0.7 [−2.9, +1.5], words +1.7 [−2.6, +6.0]. All null. Named rather than buried: Opus reserve words is **+4.89 [−2.96, +12.73]**, the largest point estimate anywhere in the comparison and still straddling zero — reserve is the noisier split (74.5 / 82.4 baseline words against 63.0 / 59.7 on shared). |
...
| `2026-08-20T12-08-19` | 600 attempted, 125 written | intermediate and advanced, both models, **the train split**, 30 repeats — **killed, and used for nothing** | Launched on the train split in the belief that it was "the five shared cases". It is not. The shared five is a convention 22 of the 24 five-case runs on disk follow, and `cases.json` records it nowhere — it differs from the train split in two of five cases. Killed as soon as the mismatch was found; **no figure in COS-18 comes from it.** Kept rather than deleted, per COS-11's rule that measured rows are excluded from means and not erased, so that a reader comparing stamps does not find an unexplained partial run between two cited ones. |
| `2026-08-20T12-12-10` | 600 | intermediate and advanced, both models, the five shared cases, 30 repeats — **COS-18's diagnostic arm** | 150 cells a style a model, **0 errored, 0 judge timeouts**. Advanced rules **99.4 / 97.3**, judge **74.3 / 68.1**; intermediate rules **95.6 / 94.7**, judge **69.5 / 72.7**. Settled which of COS-4's four defects each file carries: intermediate all four, advanced the router and (weakly) the contradiction, clean on the scoped budget and the unbounded requirement. **Its method finding outlives its numbers.** Counting over-length violations makes advanced look *worse* than intermediate — 7.7% of its Sonnet violations against 3.1%, and 7.5% against 6.1% on Opus — because advanced overruns its cap while stating it correctly, so the symptom exists without the defect. (Those shares count reply-length violations only. A first pass counted any violation mentioning words, a cap and an overrun, which swept in 20-word *sentence*-cap violations and roughly tripled every share; its figures do not reproduce and are withdrawn.) Three measures separate them: intermediate's non-status replies average **1.43× and 1.13×** its cap where advanced's average **0.95× and 0.85×**, inside it; the grader attaches the cap to *updates* in **14 of 20 and 8 of 9** of intermediate's over-length violations and in **0 of advanced's 43**, every one read by hand (see the task's adjudication rule — an earlier regex undercounted and its percentages are withdrawn); and intermediate breaks its cap about three times as often while being flagged for it about half as often (27.5% / 18.8% against 50.0% / 56.7%). The router result is the cleanest in the run: **all 138 shape violations across both files fall on non-status cases and not one on a status update**, against COS-4's 10 of 40 on beginner. |
| `2026-08-20T12-30-35` | 300 | intermediate only, both models, the five shared cases, 30 repeats — COS-18's E1 arm | The single edit re-scoping intermediate's reply cap out of its status-update section and dropping its "when things are normal" hatch: sha256 `bd326da7` → `8ea1c01b`. 150 cells a model, **0 errored**. Paired against `12-12-10` **filtered to intermediate**: rules **+3.0 [+0.1, +5.8]**, words **−24.6 [−46.4, −2.8]**, judge +2.5 [−2.8, +7.8], composite +2.8 [−0.9, +6.5]. Arm means rules 99.1 / 97.1 and judge 75.3 / 71.9 — rules at parity with advanced's 99.4 / 97.3, having sat two to four points below it on every tier of the four-tier table. Non-status mean falls 142.9 → 96.7 words on Opus and 113.0 → 84.9 on Sonnet, over-cap share 77% → 40% and 53% → 31%, and the update-scoped phrasing disappears from the grader's violations entirely. Status updates were fully compliant before and after — 0 of 120 over cap either side. **Reported and not explained: enforcement rate did not rise**, and on Opus it fell to 5.6% on 2 flagged cells of 36, which is too thin to argue from. |
| `2026-08-20T12-39-16` | 300 attempted, 136 usable | intermediate only, both models, the six reserve cases, 25 repeats — COS-18's reserve before arm, **lost** | **164 errored cells, 163 of them `You've hit your monthly spend limit`.** Sonnet lost its entire half (150 of 150, n=0); Opus kept 136 of 150. A half-arm with no Sonnet side pairs against nothing, so **no figure uses it**. The third arm this campaign has lost to a usage ceiling after session 20's 1127 cells and session 26's 56. Kept on disk on the same rule as `20-25-32`. |
| `2026-08-26T13-20-50` | 2 | intermediate, both models, `reserve-pushback`, 1 repeat | A deliberate two-cell probe, six days after the arm above, to find out whether the spend limit had reset before committing another 300 cells to it. Both cells returned clean. Cheap enough to be worth doing rather than assuming, and recorded so the practice is visible. |
| `2026-08-26T13-21-42` | 300 attempted, 141 written | intermediate only, both models, the six reserve cases, 25 repeats — COS-18's reserve before arm, retried and **stopped** | The retry after the spend limit reset. **0 errored cells** — it was not failing, it was crawling: throughput fell to roughly 0.7 cells a minute against the ~5 the same session's earlier arms ran at, which put the before arm alone about four hours out and the after arm four beyond that. Stopped by hand at 141 cells so it would burn no more quota, `complete: false`. Not used for any figure. **The consequence is that COS-18 shipped its diagnosis and not its fix**: the E1 edit was reverted, no style file changed, and the candidate moved to COS-33 with its recipe, its sha256 and its four measured intervals intact. `reserve-agentic-session` is the case that dominates the clock here, at roughly 45 seconds a cell before its judge call. |
| `2026-08-27T12-34-30` | 2 | beginner, Haiku and `claude-fable-5[1m]`, `conv-status-auth`, 1 repeat | COS-17's usage probe, run before committing 300 cells. Both cells returned, 0 errored, in 15.6s wall. The practice session 28 established, applied again and again cheap: two cells is the whole cost of finding out that the ceiling is not in force. Also the session's first throughput datum — per-cell `elapsedMs` 7345 and 8354, which is what made the two 150-cell arms obviously affordable. |
| `2026-08-27T12-35-06` | 150 | beginner, Haiku, baseline, the five shared cases, 30 repeats — **COS-17's Haiku arm** | **0 errored.** Rules **95.8**, judge **48.2**, mean reply 71.9 words (0.899× the 80-word cap). Ran 12:35:06Z to 12:50:57Z, 15m51s, **9.5 cells a minute** — beginner's replies are short and its judge calls cheap, so this arm is nothing like session 28's intermediate/advanced ones. The task's warning that Haiku aborts on write-then-verify cases did not bite: none of the five shared cases edits a file. Paired against `22-59-53` filtered to beginner × haiku × the same five cases (10 cells, 5 pairs): rules +5.0 [−6.2, +16.1], judge +10.7 [−1.0, +22.5], words −40.6 [−86.0, +4.8], composite +7.8 [−2.4, +18.1]. |
| `2026-08-27T12-50-55` | 150 | beginner, `claude-fable-5[1m]`, baseline, the five shared cases, 30 repeats — **COS-17's Fable arm** | **0 errored.** Rules **99.3**, judge **65.2**, mean reply 66.4 words (0.830× cap). 14m35s, 10.3 cells a minute. Paired against `23-48-45` filtered the same way: rules +7.9 [−6.6, +22.3], judge +11.3 [−4.8, +27.4], words −65.4 [−151.1, +20.3], composite +9.6 [−5.0, +24.2]. **Pooled with the Haiku arm over 10 case × model pairs**, which is the treatment the published figures use: rules +6.4 [−0.7, +13.5], judge **+11.0 [+3.4, +18.7]**, words **−53.0 [−91.4, −14.6]**, composite **+8.7 [+1.8, +15.6]**. One confound sits on the before side and one notably does not. **Its judge halves are clean of the cap defect**: `3370e4d` landed 13:14 UTC on 2026-08-16 and both baselines ran at 22:59 and 23:48 the same day, so these are the only before-and-after judge readings on beginner that the drift never touched — the historical row's Opus and Sonnet halves are not. Its rules halves do need a re-score, because `checks.mjs` and `contracts.json` changed again on 2026-08-17: re-deriving offline reads Haiku 91.1 and Fable 91.6 against the published 90.9 and 91.4, free and sign-preserving. And all 20 rows predate the COS-10 seam, 4 of them agentic. Restricting to the four conversational cases keeps every sign and clears none of the three intervals at 8 pairs (words −45.2 [−93.1, +2.8]). **So the deltas are directional; the load-bearing figures are the four arm means.** With these two arms the four-tier beginner row is current on every tier at 150 non-errored cells, and the provenance check that makes the columns comparable — scorer, contract, judge prompt, runner and cases all last changed before `2026-08-20T03-03-26` — is in `FINDINGS.md`. |

| `2026-08-27T14-44-59` | 2 | beginner, Opus, baseline, `conv-status-auth`, 2 repeats | COS-21's opening usage probe, run before committing any of leg 1. Both cells clean in 34.4s wall at concurrency 2. **Its per-cell reading is a trap and is recorded so the next session does not repeat it**: 34s for two parallel cells reads as ~30s a cell, which would have sized leg 1 at three hours. The arm that followed ran at **13.3s a cell**. Node startup and module load dominate a two-cell run, so a probe establishes whether the ceiling is in force and nothing about throughput. |
| `2026-08-27T14-45-48` | 150 | beginner, Opus, **baseline**, all 15 cases, 10 repeats — **COS-21 leg 1 arm 1** | **0 errored.** Rules **98.34**, judge **67.81**, composite **83.07**, mean reply 72.5 words (0.906× the 80-word cap). 14:45:48Z to 15:00:28Z — 14m40s, **10.2 cells a minute**, the same rate session 29 measured for beginner on Haiku and Fable. By split, composite holdout 86.0 / train 84.1 / reserve 80.2. This is the control every other arm in COS-21's beginner × Opus leg pairs against. |
| `2026-08-27T15-00-47` | 150 | same, **long-prompt** — leg 1 arm 2 | **0 errored**, 14m58s. Rules 97.61, judge 66.07, composite 81.84, 76.4 words. Paired against the baseline arm over 15 case pairs: rules −0.7 [−1.5, +0.1], judge −1.7 [−7.2, +3.7], composite −1.2 [−4.0, +1.5], words +3.4 [−1.6, +8.4]. **Every interval contains zero.** The ADR puts this variant at −4.5 points, measured on advanced × Opus at five cells an arm. |
| `2026-08-27T15-15-54` | 150 attempted, 29 usable | same, **tail-reminder** — **LOST** | **121 errored, every one `You've hit your monthly spend limit`.** Ran 3m20s, which is the tell: a healthy 150-cell beginner arm takes about fifteen minutes, so one that finishes in three has mostly not run. 29 usable cells is below COS-21's 146 floor and **no figure uses this directory**; kept on disk under COS-11's rule that errored cells are excluded from means, not erased. The fourth arm this campaign has lost to a usage ceiling, after `20-25-32`, `12-39-16` and `13-21-42`. Note which ceiling: the session was pacing against the 5-hour window and had spent 34 minutes of it. **The monthly spend limit is a different limit and that budget does not protect against it.** |
| `2026-08-27T21-10-10` | 2 | beginner, Opus, baseline, `conv-status-auth`, 2 repeats | The re-probe after a ~6-hour pause, before spending anything on the retry. Both cells clean, 37s. Third time this practice has paid for itself in three sessions. |
...
**COS-17 adds 302 cells** — a 2-cell probe and two clean 150-cell arms, all three
0 errored, all three load-bearing. That would put the running figure at 6910.

**It does not, and the gap is prior to this session.** Counting `rows.json`
lengths across every directory under `harness/results/` returns **6996 rows in 89
directories** — 6284 under a `run` manifest, 4 under an `improve` manifest, and
708 in 36 pre-manifest directories. Net of the 4 improve-loop cells, which this
total has always excluded by the rule in the paragraph above, that is **6992
against an expected 6910: 82 cells on disk that the running total does not
account for.** The discrepancy predates COS-17 and this entry does not resolve it;
it is recorded here rather than papered over by writing 6910, and rather than
silently replacing the figure with 6992, because which of the two is wrong has not
been established. Re-deriving the total from disk is free — the count above is one
`node -e` over `results/*/rows.json` — and a session that needs the number should
do that rather than trust either.
...
session's own 904 gives 6996 in 89 — reproducing session 29's figure exactly, so
the two counts agree on what is there and disagree only on how it is classified.
The earlier count put 4 cells "under an improve manifest" and 708 in "36
pre-manifest directories". Splitting that second bucket by manifest shows **9 of
those 36 directories are not pre-manifest at all: they carry an `improve.json`
and no `run.json`, and they hold 70 rows**, every one from 2026-08-16, which is
when COS-3 started persisting improve-loop rows. The classifier read a missing
`run.json` as "no manifest" and swept improve-loop cells into the total that this
ledger has always excluded them from. Excluding all 74 improve-loop rows rather
than 4 puts the pre-COS-21 disk figure at **6922 against an expected 6910 — a
12-cell gap, not 82.** The remaining 12 are still unexplained and are left open.

**Current total, counted rather than accumulated: 7900 rows in 97 directories,
7826 net of the 74 improve-loop cells.** Anyone needing this number should
re-derive it the same way — the count is one `node -e` over `results/*/rows.json`
that classifies each directory by which manifest it carries.
