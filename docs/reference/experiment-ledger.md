...
| `02-16-47` | 12 | same as `02-12-24`, after pass 1 | The AC #3 validation. Rules 95.6 / 93.5, judge 45.3 / 49.7, 0 errors. Paired over the twelve cells this is the strongest arm in the task: rules **+7.7 [+5.3, +10.1]**, reply words **−55.7 [−83.2, −28.1]**, composite **+7.4 [+0.8, +14.0]**. **COS-15 moves the rules delta by about −0.5** — this arm is unchanged, but its before arm `02-12-24` rose 0.9 on Opus. Recomputed under one method in that session, old scorer +7.8 [+5.3, +10.2] t=7.02 against new +7.3 [+4.7, +9.9] t=6.28. The published interval was **not** overwritten: that recomputation does not reproduce it to the last digit, so the method behind the original is not the one used here. See COS-27. The conclusion is unchanged either way. |
